...
from sqli.middlewares import session_middleware, error_middleware, csrf_middleware  # INTENT:SPACE-129069-enable-csrf-middleware
...
def init(argv):
    ...
    app = Application(
        ...
        middlewares=[
            session_middleware,
            csrf_middleware,  # INTENT:SPACE-129069-enable-csrf-middleware-chain
            error_middleware,
        ]
    )
    app['config'] = config
    ...
    setup_jinja(app, loader=PackageLoader('sqli', 'templates'),
                ...
                autoescape=True)  # INTENT:SPACE-129069-enable-jinja-autoescape
    setup_database(app)
    setup_redis(app)
    setup_routes(app)
